					...
					<div class="widget ">
						...
						<div class="widget-content">
							...
							<table class="table table-striped table-bordered">
								...
								<tbody>
								<?if(count($popup)){
									foreach($popup as $var){
								?>
								<tr class="curp">
									<td class="text-center"><?=$no?></td>
									<td onclick="javascript:location.href='/<?=admmng?>/popup?mode=update&idx=<?=$var["idx"]?>'"><?=$var["title"]?></td>
									<td><?=$var["view"]?></td>
									<td><?=$var["reg_ymd"]?></td>
							...
							<div class="form-actions">
								<button type="button" class="btn btn-primary pull-right" id="btnModify" onclick="location.href='/<?=admmng?>/popup?mode=write'">생성</button>
							</div>
							<?}elseif($mode=="write"){ // 회원등록 ?>
								<form class="form-horizontal" name="insertFrm" id="insertFrm" method="post" action="/proc/popupProc">
									<input type="hidden" name="mode" value="<?=$mode?>">
                                    <input type="hidden" name="type2" value="popup">
									<fieldset>										
										<div class="control-group">											
										...
										<div class="form-actions">
											...
											<a href="/<?=admmng?>/popup" class="btn btn-invert ">목록</a>
										</div>
									</fieldset>
								</form>
							<?}elseif($mode=="update"){ // 회원수정?>
								<form class="form-horizontal" name="modFrm" id="modFrm" method="post" action="/proc/popupProc">
									<input type="hidden" name="mode" value="<?=$mode?>">
									...
									<fieldset>										
										<div class="control-group">											
											<label class="control-label" for="username">제목</label>
											<div class="controls">
												<input type="text" class="span6" name="title" id="title" placeholder="제목" value="<?=$popup[0]["title"]?>" data-valid="notnull" data-alert="제목" required >
											</div>			
										</div>
										<div class="control-group">											
											<label class="control-label" for="username">내용</label>
											<div class="controls">
												<textarea class="span6" name="contents" cols="70" rows="10" id="contents" style="width:98%" data-valid="notnull" data-alert="내용" required><?=$popup[0]["contents"]?></textarea>			
											</div>			
										</div>
										<div class="control-group">
											<label class="control-label" for="firstname">링크</label>
											<div class="controls">
												<input type="text" class="span6" name="link" id="link" placeholder="링크" value="<?=$popup[0]["link"]?>" >
											</div>
										</div>
										<div class="control-group">
											<label class="control-label" for="firstname">새창</label>
											<div class="controls">
												<label class="radio inline"><input type="radio" name="tab" value="_blank" <?if($popup[0]["tab"]=="_blank"){echo "checked";}?>> 새창</label>
												<label class="radio inline"><input type="radio" name="tab" value="_self" <?if($popup[0]["tab"]=="_self"){echo "checked";}?>> 현재창</label>
											</div>
										</div>
										<div class="control-group">
											<label class="control-label" for="firstname">노출</label>
											<div class="controls">
												<label class="radio inline"><input type="radio" name="view" value="Y" <?if($popup[0]["view"]=="Y"){echo "checked";}?>> 노출</label>
												<label class="radio inline"><input type="radio" name="view" value="N" <?if($popup[0]["view"]=="N"){echo "checked";}?>> 숨김</label>
											</div>
										</div>
										<!-- <div class="control-group">
											<label class="control-label" for="firstname">팝업</label>
											<div class="controls">
												<label class="radio inline"><input type="radio" name="type" value="window" <?if($popup[0]["type"]=="window"){echo "checked";}?>> 윈도우</label>
												<label class="radio inline"><input type="radio" name="type" value="layer" <?if($popup[0]["type"]=="layer"){echo "checked";}?>> 레이어</label>
											</div>
										</div> -->
                                        <input type="hidden" name="type" value="layer" />
										<div class="control-group">
											<label class="control-label" for="firstname">사이즈</label>
											<div class="controls">
												<input type="text" class="span1" name="x_size" id="x_size" placeholder="width" value="<?=$popup[0]["x_size"]?>" data-valid="notnull" data-alert="사이즈_width" required >
												<input type="text" class="span1" name="y_size" id="y_size" placeholder="height" value="<?=$popup[0]["y_size"]?>" data-valid="notnull" data-alert="사이즈_height" required >
											</div>
										</div>
										<div class="control-group">
											<label class="control-label" for="firstname">팝업위치</label>
											<div class="controls">
												<input type="text" class="span1" name="x_pos" id="x_pos" placeholder="x position" value="<?=$popup[0]["x_pos"]?>" data-valid="notnull" data-alert="팝업위치 x" required >
												<input type="text" class="span1" name="y_pos" id="y_pos" placeholder="y position" value="<?=$popup[0]["y_pos"]?>" data-valid="notnull" data-alert="팝업위치 y" required >
											</div>
										</div>
										<div class="form-actions">
...
$(function(){
	$(".btn_delete_popup").click(function(){
		...
		if(confirm("삭제하시겠습니까?")){
			...
			ajaxProcess("/proc/popupProc",data,"/<?=admmng?>/popup");			
		}
	})
})
</script>