...
<?
    ...
    $good_name      = $_POST[ "good_name"      ]; // 상품명
	  $good_name = iconv("utf8", "euckr", $good_name);
	...
	$buyr_name = iconv("utf8", "euckr", $buyr_name);
    ...
    $orderno   = $_POST[ "orderno"   ];	// 모든데이타
    $period   = $_POST[ "period"   ];
    $price   = $_POST[ "price"   ];
    $discount   = $_POST[ "discount"];
    $total_price   = $_POST[ "total_price"];
    $pay_method   = $_POST[ "pay_method"];
	if($pay_method=="100000000000"){$pay_method = "CARD";}
	if($pay_method=="010000000000"){$pay_method = "BANK";}
	if($pay_method=="001000000000"){$pay_method = "VBANK";}
    ...
    if ( $req_tx == "pay" )
    {
            ...
            //$c_PayPlus->mf_set_ordr_data( "ordr_mony", $friend_count * $cost );
...
// exit;
    ...
    /* = -------------------------------------------------------------------------- = */
            ...
            if ( $use_pay_method == "100000000000" )
            {
				...
				$bank_name = $card_name;  // 은행명
                $bank_code = $card_cd;  // 은행코드
                $bk_mny = $card_mny;			// 계좌이체결제금액
            }
    ...
    if ( $req_tx == "pay" )
    {
        if( $res_cd == "0000" )
        {
            // 06-1-1. 신용카드, 계좌이체, 가상계좌
            if ( $use_pay_method == "100000000000" ||  $use_pay_method == "010000000000" ||  $use_pay_method == "001000000000")
            {
				$bSucc = "true";
            }
			$pay_status = "DONE";
...
$res_msg = iconv("utf8", "euckr", $res_msg);
echo "res_msg : ".var_dump($res_msg);
    /* ============================================================================== */
    /* =   08. 폼 구성 및 결과페이지 호출                                           = */
    /* ============================================================================== */
?>
...
<body onload="goResult()">
    <form name="pay_info" method="post" action="/payment/payment_card_proc">
        ...
        <input type="hidden" name="res_msg"           value="<?=urlencode($res_msg)        ?>">    <!-- 결과 메세지 -->
        ...
        <input type="hidden" name="good_name"         value="<?=urlencode($good_name)      ?>">    <!-- 상품명 -->
        ...
        <input type="hidden" name="card_name"         value="<?=urlencode($card_name)      ?>">    <!-- 카드명 -->
        ...
        <input type="hidden" name="bank_name"         value="<?=urlencode($bank_name)?>">    <!-- 은행명 -->
		...
		<input type="hidden" name="username"		value="<?=$buyr_name   ?>">
		<input type="hidden" name="userhp"		value="<?=$buyr_tel1   ?>">
		<input type="hidden" name="useremail"		value="<?=$buyr_mail   ?>">
		<input type="hidden" name="pay_type"		value="<?=$pay_method    ?>">
		<input type="hidden" name="bk_mny"		value="<?=$bk_mny   ?>">
		<input type="hidden" name="pay_status"		value="<?=$pay_status?>">
		<input type="hidden" name="via"		value="PC">
